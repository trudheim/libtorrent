...
ai_each_inet_inet6_first(const char* nodename, ai_sockaddr_func lambda, int flags) {
  ...
  if ((err = ai_get_addrinfo(nodename, NULL, ai_make_hint(0, AF_INET, SOCK_STREAM).get(), ai)) != 0 &&
      (err = ai_get_addrinfo(nodename, NULL, ai_make_hint(0, AF_INET6, SOCK_STREAM).get(), ai)) != 0)
    return err;
